   ...
   <header className='w-full bg-transparent fixed top-0 left-0 right-0 transition-all ease-in duration-300'>
    <nav className={`py-4 lg:px-24 px-4 ${isSticky ? "sticky top-0 left-0 right-0 bg-green-300" : ""}`}>
        <div className='flex justify-between items-center text-base'>
            ...
            <FaBlog className='inline-block'/>Books World</Link>
            ...
            <ul className='md:flex space-x-12 hidden'>
                {
                    navItems.map(({link,path})=><Link key={path} to={path}
                     className='block text-base text-black uppercase cursor-pointer hover:text-green-600'>
                        {link}</Link>)
                }
            </ul>
                 ...
                 <div className='space-x-12 hidden lg:flex items-center'>
                    <button><FaBarsStaggered className='w-5 hover:text-green-600'/></button>
                      ...
                      <div className='md:hidden'>
                        <button onClick={toggleMenu} className='text-black focus:outline-none'> 
                          ...
                          <div className={`space-y-4 px-4 mt-16 py-7 bg-green-600 
